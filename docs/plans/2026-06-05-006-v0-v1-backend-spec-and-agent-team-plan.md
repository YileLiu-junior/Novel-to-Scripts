...
| S5 | Persistence 与 artifact | 保存 job、artifact、llm trace | `backend/app/db/`、`repositories/` | Backend Builder |
...
### U5: SQLite、Repository 与 Artifact
...
- `artifacts.version` 从 1 开始递增。
- `llm_runs` 即使 fake provider 也要能写占位 trace，方便未来接真实模型。
...
- 每一步失败都写入 `generation_jobs.error`。
- 每一步成功都写入 artifact。
...
| Skill Engineer | provider、skill wrapper、prompt、fake response | 保存数据库、决定 job 状态 |
...
- 生产化：SQLite 换 PostgreSQL，BackgroundTasks 换 RQ/Celery，但保留 repository 和 worker 边界。
