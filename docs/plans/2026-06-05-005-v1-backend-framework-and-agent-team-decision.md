...
```text
Python + FastAPI + Pydantic v2 + SQLite + artifact/job pipeline
```
...
- V1 用 FastAPI `BackgroundTasks` 和 job 状态表，不引入 Redis/Celery。
...
| 旧机制 | V1 后端映射 | 用途 |
| --- | --- | --- |
| `outputs/{剧本名}/.agent-state.json` | `generation_jobs` | 保存当前生成任务状态 |
| agentId resume | `artifact.version` + `source_job_id` | 从上一步产物恢复 |
| agent 日志 | `llm_runs` + backend logs | 追踪模型调用和解析错误 |
