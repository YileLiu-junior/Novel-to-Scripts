...
用户真正需要感知的不是“系统多跑了一个 skill”，而是“剧本不是黑盒生成”。当一个 scene 出现时，用户应该能追到它来自哪些原文事件、哪些关系和知情差被保护、哪些事件不能被拆开、哪些信息必须影视化表达。
...
- R3. 文案应避免把 ontology 作为主界面术语；可以使用“改编证据”“故事圣经”“原文结构档案”等面向创作者的说法。

**后端输入输出**

- R4. 后端生成请求应支持一个可见开关，用于控制 StoryOntology 是否输出 V1.5 改编证据层；开关默认启用，但不能绕过主 pipeline。
...
- 只引入少量 V1.5 字段：优先完整事件、一致性锚点、冲突轴和可影视化约束，暂缓更复杂的爽点、反转和长集数结构。
- 开关只控制 evidence 丰富程度，不允许跳过 StoryOntology：主链路仍然稳定保存 `story_bible` artifact。
...
- [Affects R4][Technical] 开关应落在 `adaptation_config` 内，还是独立于生成请求 DTO。
