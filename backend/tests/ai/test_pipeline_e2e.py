...
# Default novel, overridable via E2E_NOVEL_PATH env var.
_NOVEL_PATH = Path(
    os.getenv(
        "E2E_NOVEL_PATH",
        _REPO_ROOT
        / "research" / "novel-to-script-cases"
        / "03_pride_and_prejudice_2005"
        / "pride-and-prejudice_novel_zh.txt",
    )
)
...
def _split_novel_for_e2e(
    ...
    """Split raw novel text into chapter chunks using the production ChapterSplitter.

    Delegates to the unified rule-based ChapterSplitter, then converts
    SplitChapter objects to dicts for ChapterService.normalize_chapters().
    """
    from app.services.chapter_splitter import ChapterSplitter

    splitter = ChapterSplitter()
    split_chapters = splitter.split(text, mode="rule")

    chunks: list[dict[str, str]] = []
    for ch in split_chapters:
        chunks.append({"title": ch.title, "text": ch.text})
...
def _read_novel_text() -> str:
    """Read the novel file, returning the first ~300KB for E2E testing.

    Pride and Prejudice has a long preface (~30KB), so we need more to
    capture 3-4 real chapters.
    """
...
class PipelineE2ETest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        _stem = _NOVEL_PATH.stem[:20]  # e.g. "pride-and-prejudice_novel_zh" → "pride-and-prejudice_no"
        cls.project_id = f"E2E-{_stem[:12]}-{_ts}"
        cls.project_svc = ProjectService()
        cls.project = cls.project_svc.create_project(
            title=f"{_stem}-{_ts}",
            logline="E2E pipeline test run",
    ...
    def test_run_v1_pipeline_with_real_ai(self) -> None:
        """E2E: run_v1() processes first 3-4 chapters of Pride and Prejudice."""
        raw_text = _read_novel_text()
        chunks = _split_novel_for_e2e(raw_text, max_chapters=4)
