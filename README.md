  ...
  characters:
    - id: char_001
      ...
      voice_profile:
        rhythm: "观察细致，叙述克制"
  relationship_edges: [...]
  continuity_anchors: [...]     # 不可漂移的事实锚点
  dramatic_assets:              # 冲突池 + 影视化约束
    conflict_pool: [...]
    filmic_constraints: [...]
...
foreshadowing:                  # 伏笔追踪（setup → payoff）
  - id: foreshadow_001
    setup_event_id: event_003
    status: "open"

adaptation_plan:                # 改编决策（保留/合并/删除事件 + 场景规划）
...
scenes:                         # ★ 真正剧本内容 ★
  - id: scene_001
    ...
    content_blocks:             # 自然段正文（动作 + 对白交错排列）
      - id: block_001
        block_type: "action"
        text: "华生打量房间..."
      - id: block_002
...
> **YAML 是中间格式** — 额外附加了来源引用、潜台词、角色声音等元数据。可渲染为文学剧本排版（Markdown / 纯文本）。
